...
//                echo '<pre>'; var_dump($sql_events);
        ...
        if($notDeals){
            $closed_won = "AND opportunities.id IN (SELECT parent_id 
                                                    ...
                                                    AND opportunities_audit.date_created BETWEEN '$datetime_from' AND '$datetime_to' 
                                                    AND opportunities_audit.after_value_string = 'Shipment expectation'
                                                    AND opportunities.sales_stage NOT IN ('Closed Won')
                                                    )";
        }else{
        ...
        $sql_deals ="SELECT CONCAT(users.last_name, ' ', users.first_name) AS full_name,
                           ...
                           -- ROUND(AVG(productsale.id),0) AS avg_product
                            ROUND(COUNT(productsale.id) / COUNT(DISTINCT opportunities.id),0) AS avg_product
                    FROM opportunities
...
//        echo '<pre>'; var_dump($sql_deals);
        ...
        foreach ($users as $users_arr) {
            ...
            // Цикл для перебора значений переменной $result_deals
